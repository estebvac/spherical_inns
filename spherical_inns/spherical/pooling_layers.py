...
class HealpixMaxPool(nn.MaxPool1d):
    ...
    def forward(self, x, index=None):
        ...
        Args:
            ...
            index:  Not use only for make it compatible with all pooling classes
        Returns:
...
class HealpixPooling(nn.Module):
    """Healpix class, which groups together the corresponding pooling and unpooling.
    """
    ...
    def __init__(self, mode="average"):
        super().__init__()
        if mode == 'max':
            self.pooling_layer = HealpixMaxPool()
        if mode == 'mean':
            self.pooling_layer = HealpixAvgPool()
        if mode == 'un_max':
            self.pooling_layer = HealpixMaxUnpool()
        if mode == 'un_mean':
            self.pooling_layer = HealpixAvgUnpool()

    def forward(self, x, index=None):
        return self.pooling_layer(x, index)

...
def cheb_conv(laplacian, inputs, weight):
    ...
    if K > 0:
        x1 = laplacian.mm(x0)
        # x1 = spmm(laplacian.indices(), laplacian.values(), laplacian.size()[0], laplacian.size()[1], x0)  # V x Fin*B
        ...
        for _ in range(1, K - 1):
            ...
            # x2 = 2 * spmm(laplacian.indices(), laplacian.values(), laplacian.size()[0], laplacian.size()[1], x1) - x0
            x2 = 2 * laplacian.mm(x1) - x0
...
class SphericalChebConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, lap, kernel_size):
        ...
        """
        super().__init__()
        self.register_buffer("laplacian", lap.to_dense())
    ...
    def forward(self, x):
        ...
        """
        self.laplacian = self.laplacian.to(x.device).to(x.dtype)
